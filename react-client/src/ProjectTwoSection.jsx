...
const ModalTwo = props => (
  <div className={props.status ? 'modal is-active' : 'modal'}>
    ...
    <button onClick={() => props.close()} className="modal-close is-large" aria-label="close" />
  </div>
);
...
<div onClick={()=> props.close()}>
  ...
  <section className="section section-dark">
    ...
    to replicate a portion of the Airbnb Experiences microservice architecture that would allow us
    to handle working with millions of concurrent users and monitor data flow between each of our
    separate microservices. Each service had specific goals and needed to maintain a certain level
    of efficiency so that no one service would create a bottleneck for the entire team.
    </h4>
    <br/>
    <h4 className="has-text-left has-text-white-ter words"> The client service and aggregator
    service both relied on my service to operate successfully. Whenever a valid user visited
      the client, a request would be made to my microservice asking for that user’s recent search
      history so that the client service could populate the users view with relevant locations.
      Anytime a user searched for a location, the client would make a request to my service so I
      could check to see if this was a newly searched location by the specific user. If it was a new
      search, I would then send the location to the aggregator service so it could keep track of
      most search locations.
    </h4>
    <br/>
    <h4 className='has-text-left has-text-white-ter words'> All of my data had relationships with one or more other data points so a
      <strong className='has-text-white has-text-weight-bold bigText'> relational database </strong> was necessary. I made the decision to use
      <strong className='has-text-white has-text-weight-bold bigText'> MariaDb</strong> as my database of choice do to its fast reading speeds. Its writing speeds were not as fast as other
      relational databases, but it was a tradeoff I was willing to take since my service was mainly responsible for relaying
      user info to other services. In order to relay information from microservice to microservice, multiple
      <strong className='has-text-left has-text-white-ter words bigText'>
      Amazon Simple Queue Services </strong> were used to exchange data. </h4>
  </section>
  ...
  <section className="section section-dark">
    ...
    <h4 className='has-text-left has-text-white-ter words'>
      Upon building out my microservice and ensuring everything was fully functional, the next step was to see how my microservice
    ...
    <br/>
    <h4 className='has-text-left has-text-white-ter words'>
      My first optimization involved restructuring my database query logic and converting a six step query into a four step one.
      This did improve my response per second time, but not significantly enough to be satisfied with.
      ...
      <div className="columns">
        <div className="column">
          <div className="video-responsive">
            <iframe width="560" height="315" src="https://www.youtube.com/embed/8zyRCkLPjO0?rel=0" frameBorder="10" allowFullScreen></iframe>
          </div>
        </div>
      </div>
